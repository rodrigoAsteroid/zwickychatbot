...
const ChatButton: React.FC<ChatButtonProps> = ({ onClick, isOpen }) => {
  return (
    <button
      className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-[#F27F04] flex items-center justify-center shadow-lg z-50 hover:bg-[#CC710F]"
      onClick={onClick}
      aria-label={isOpen ? "Cerrar chat" : "Abrir chat"}
    >
      {!isOpen ? (
        // Icono de Zwicky para el botón cuando el chat está cerrado
